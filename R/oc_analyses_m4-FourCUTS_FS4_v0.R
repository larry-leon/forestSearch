...
# Default parameters
oc_analyses_m4FourCuts_FS4<-function(sim){
...
if(sim <= 1 | sim>=(Nsims-1)) showsim <- TRUE
  ...
  if(get.FS){
  # "FSl" but here with NO lasso but GRF  
    use_lasso <- FALSE
    use_grf <- TRUE
    ...
    y.fs<-try(forestsearch(df.analysis=x, Allconfounders.name=confounders.name,
                        ...
                        conf_force=NULL, 
                        defaultcut_names=confounders.name,
                        ...
                        plot.sg=FALSE,vi.grf.min=vi.grf.min),TRUE)
...
# NOTE, the following are contained in oc_analyses_m4_FS4_v0()
# May want to move to an independent file, but for now comment out
# 
