...
# May be able to speedup by removing excess weighing calcs for rho=gamma=0
# Requires cox_rhogamma_functions (NOT currently loaded)
# At this point just use coxph() 
#get.Cox<-function(time,delta,z){
#fit.rg<-cox.rhogamma(time=time,delta=delta,z=z,rho=0,gamma=0,t.tau=NULL,w0.tau=0,w1.tau=1,est.method="score")
#bhat<-fit.rg$bhat
