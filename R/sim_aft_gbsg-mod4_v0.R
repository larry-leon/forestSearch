...
return(list(df.super_rand=df.super_rand,sigC=sigC,hr.H.true=hr.H.true,hr.Hc.true=hr.Hc.true,cens.type=cens.type,mu=mu,hr.causal=hr.causal,b.true=b.true,
...
get.dgm4<-function(mod.harm,N,k.treat,hrH.target,cens.type,out.loc,file.index,model.index,details=FALSE,parms_torand=FALSE,sol_tol=10^-8){
  k.inter<-NULL  
  if(mod.harm!="null"){ 
  ...
  if(mod.harm=="alt"){
    ...
    out.file<-paste0(out.file,inter.id,sep="_")
  }
