...
outfs<-c("output/actg_2v3_fs-Nsg.Rdata")
# Boots=2000
outfsboot<-c("output/actg_2v3_fsboot-Nsg_B=2000.Rdata")
...
\begin{figure}[h!]
\begin{center}
<<ACTG_2v3_grf-fsNsg_sg,echo=TRUE,out.height="400px",out.width="400px">>=
# Compare with GRF
layout(matrix(c(1,2,3,4), 2, 2, byrow = TRUE))
plot.subgroup(sub1=df0.grf,sub1C=df1.grf,tte.name="time_days",event.name="cens",treat.name="treat",fix.rows=FALSE,
              byrisk=200,show.med=FALSE,ymin=0.4,subtitle1="(a)",subtitle2="(b)",xlab="Days")
plot.subgroup(sub1=df0.fs,sub1C=df1.fs,tte.name="time_days",event.name="cens",treat.name="treat",fix.rows=FALSE,
              byrisk=200,show.med=FALSE,ymin=0.4,subtitle1="(c)",subtitle2="(d)",xlab="Days")
@
\end{center}
\end{figure}
...
caption="\\label{tab:actg} ACTG-175 FS-Nsg Analysis: Cox hazard ratio (HR) estimates for the ITT population and subgroups $H$ and $H^{c}$.
